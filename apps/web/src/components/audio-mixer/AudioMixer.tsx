...
const MasterChannel: React.FC<{
  ...
  return (
    <div className="flex flex-col items-center gap-2 p-3 bg-gray-900 rounded-lg min-w-[100px] border border-gray-700">
      <Text type="label" weight="bold" className="text-xs text-gray-300">
        {t("common:audioMixer.master", "MASTER")}
      </Text>
      ...
      <div className="flex flex-col items-center gap-1">
        ...
        <Slider
          label={t("common:audioMixer.masterVolumeFader", "Master volume fader")}
          isLabelHidden
          min={0}
          max={4}
          step={0.01}
...
export const AudioMixer: React.FC<AudioMixerProps> = ({
  ...
  return (
    <div
      ...
      role="region"
      aria-label={t("common:audioMixer.console", "Audio Mixing Console")}
    >
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <Text type="body" weight="semibold" className="text-lg text-white">
          {t("common:audioMixer.title", "Audio Mixer")}
        </Text>
        {onClose && (
          <IconButton
            label={t("common:audioMixer.close", "Close mixer")}
            onClick={onClose}
      ...
      <div className="mt-3 pt-3 border-t border-gray-800 flex items-center justify-between text-xs text-gray-500">
        <span>
          {t("common:audioMixer.channelCount", {
            count: channels.length,
            defaultValue: channels.length === 1 ? "{{count}} channel" : "{{count}} channels",
          })}
          {hasSoloedTracks && (
            <span className="ml-2 text-yellow-500">
              {t("common:audioMixer.soloActive", "• Solo active")}
            </span>
          )}
        </span>
        <span>
          {t("common:audioMixer.sampleRateInfo", {
            rate: project.settings.sampleRate,
            channels: project.settings.channels,
            defaultValue: "Sample Rate: {{rate}}Hz | Channels: {{channels}}",
          })}
        </span>
      </div>
    </div>
