  ...
  const handleImport = useCallback(() => {
    ...
    try {
      ...
      if (importedProject) {
        ...
        if (missingCount > 0) {
          toast.warning(
            t("scriptView.assetsNeedRelinking", "{{count}} asset needs relinking", { count: missingCount }),
            t("scriptView.relinkHint", 'Go to Assets panel → click "Relink from Folder" to restore missing media.'),
          );
        }
      }
    } catch (error) {
  ...
  return (
    <Dialog isOpen onOpenChange={(open) => !open && onClose()} width={896} purpose="form">
      <Layout
        header={
          <DialogHeader
            title={t("scriptView.title", "Project JSON")}
            subtitle={t("scriptView.subtitle", "Export or import project as JSON")}
        ...
        <div className="border-b border-border p-2">
          <ToolcraftSegmentedControl<"export" | "import">
            ...
            options={[
              { value: "export", label: t("scriptView.exportTab", "Export JSON") },
              { value: "import", label: t("scriptView.importTab", "Import") },
            ]}
          />
        </div>
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
              ...
              {exportedJson ? (
                <>
                  <div className="flex gap-2 p-3 border-b border-border">
                    <Button
                      label={copySuccess ? t("scriptView.copied", "Copied!") : t("scriptView.copy", "Copy")}
                      variant="secondary"
                      size="sm"
                      icon={
                        copySuccess ? (
                    ...
                    />
                    <Button
                      label={t("scriptView.downloadJson", "Download JSON")}
                      variant="secondary"
                      size="sm"
                ...
                <div className="flex-1 flex flex-col items-center justify-center gap-3 p-8 text-center">
                  ...
                  <Text type="body" color="secondary">
                    {t("scriptView.noData", "No project data to export.")}
                  </Text>
                </div>
              )}
            </>
          ...
          {activeTab === "import" && (
            <div className="flex-1 flex flex-col gap-4 p-4 overflow-auto">
              <FileInput
                label={t("scriptView.fileInputLabel", "Project JSON file")}
                isLabelHidden
                value={null}
                onChange={(picked) => {
                  if (picked instanceof File) {
                    if (picked.type === "application/json" || picked.name.endsWith(".json")) {
                      ...
                      setValidation({
                        valid: false,
                        errors: [t("scriptView.invalidFileType", "Please upload a .json file")],
                        warnings: [],
                      });
                    }
                  }
                ...
                mode="dropzone"
                placeholder={t("scriptView.dropPlaceholder", "Drop a JSON file here or click to browse")}
                description={t("scriptView.dropDescription", "Accepts .json project files")}
                width="100%"
              />
              ...
              {importJson && (
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <Button
                    label={isValidating ? t("scriptView.validating", "Validating...") : t("scriptView.reValidate", "Re-validate")}
                    variant="secondary"
                    size="sm"
                  ...
                  {validation.valid && (
                    <Card variant="green" padding={3} className="flex items-center gap-2 border border-primary/30">
                      ...
                      <Text type="body" className="text-primary">
                        {t("scriptView.validJson", "Valid project JSON — ready to import")}
                      </Text>
                    </Card>
                  )}
                  ...
                  {validation.warnings.length > 0 && (
                    <Card variant="muted" padding={3} className="space-y-1 border border-warning/30 bg-warning/10">
                      <div className="flex items-center gap-2 text-warning font-medium text-sm">
                        ...
                        {t("scriptView.warnings", "Warnings")}
                      </div>
                    ...
                    validation.missingAssets.length > 0 && (
                      <Card variant="muted" padding={3} className="space-y-1 border border-border">
                        <Text type="body" color="secondary" weight="bold" display="block" className="text-sm">
                          {t("scriptView.missingAssets", "Missing Assets ({{count}})", { count: validation.missingAssets.length })}
                        </Text>
                        <Text type="supporting" color="secondary" display="block">
                          {t("scriptView.missingAssetsDesc", "These assets will be imported as placeholders and can be replaced later.")}
                        </Text>
                      </Card>
                    )}
                </div>
              ...
              {importJson && (
                <Button
                  label={t("scriptView.importProject", "Import Project")}
